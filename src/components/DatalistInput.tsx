...
const useComboboxHelpers = ({
	...
	const handleClickOutside = useCallback(
		(e: MouseEvent) => {
			...
			if (
				!targetElement ||
				(!listboxRef.current.contains(targetElement as HTMLElement) &&
					targetElement !== comboboxInputRef.current)
			) {
				handleCloseDatalist();
			}
		},
	...
	const handleFocusOutside = useCallback(() => {
		...
		if (
			!activeElement ||
			(!listboxRef.current.contains(activeElement) && activeElement !== comboboxInputRef.current)
		) {
			handleCloseDatalist();
		}
	}, [handleCloseDatalist]);
...
const startsWithValueFilter: Filter = (items, value = "") =>
	items.filter((item) =>
		typeof item.value === "string"
			? item.value.substring(0, value.length).toLowerCase() === value.toLowerCase()
			: true
	);
